...
import { ICookie, IOverrideRequest, IPagination } from 'common.interfaces';
import { Response } from 'express';
import httpStatusCodes from 'http-status-codes';

export default class ApiResponse {
  static result = (res: Response, data: object, status: number = 200, cookie: ICookie = null, pagination: IPagination = null) => {
    res.status(status);
    if (cookie) {
